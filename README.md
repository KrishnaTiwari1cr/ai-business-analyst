...
the application displays a formatted KPI.
...
the application generates a category comparison visualization.
...
the application generates a time-series visualization.
...
SQL Execution Recovery

Database errors can trigger automatic SQL repair.

Retry Limiting

SQL repair attempts are bounded to prevent infinite loops.

Empty Response Handling

The application checks for empty responses from LLM providers.

Visualization Fallback

The visualization layer can use suitable query results when dedicated visualization data is unavailable.
